...
public class ScrappedGameFromGrid {

    private final Element game;

    public ScrappedGameFromGrid(Element element) {
        this.game = element;
    }
    ...
    private Optional<ScrappedDetailGame> goTo(){
        return getDetailURL().map(url -> {
            try {
                return new ScrappedDetailGame(Jsoup.connect(url).userAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36")
